#Updated 2/6/2020 by Emma Morgan (emma.morgan@tufts.edu)
...
dictionary_years_to_index <- function(dictionary_unique) {
  dictionary_unique_modified <- dictionary_unique %>%  
    ...
    mutate(reference_ay = if_else(str_detect(VARTITLE,"[:digit:]{4}-[:digit:]{2}"),
                                  as.integer(str_extract(VARTITLE,"[:digit:]{4}(?=-[:digit:]{2})"))+1,
                                  -99)) %>%
    mutate(reference_index = if_else(reference_ay != -99, as.integer(ACAD_YEAR) - reference_ay, -99)) %>%
    
    #Now use the reference index to replace as needed.
    mutate(VARTITLE_USE = map2_chr(reference_index, 
                                   VARTITLE, 
                                   ~ case_when(.x==0 ~ str_replace(.y, "[:digit:]{4}-[:digit:]{2}", "(current year)"),
                                               .x == 1 ~ str_replace(.y, "[:digit:]{4}-[:digit:]{2}", "(prior year)"),
                                               .x >= 2 ~ str_replace(.y, "[:digit:]{4}-[:digit:]{2}", paste0("(",as.character(.x)," years prior)")),
