...
```{r gauth}
pkgs <- c("tidyverse", "knitr", "rmarkdown", "readr","styler", "kableExtra", "janitor", "likert", "readxl", "gt", "vroom", "ggtext", "DT","lubridate", "stringr", "scales", "htmltools")
for(pkg in pkgs) {
...
```{python scrape}
#| eval: false
#| engine-path: C:/Users/kaloisio/Documents/GitHub/IPEDS_longitudinal/prep ipeds trend data/env/Scripts/python.exe
#| include: false
...
def which_file_version(data_dir):
	...
	for ziparchive in glob.glob(file_search_path):   
		...
		for file_name in file_list:
			if "_rv" in file_name or "_RV" in file_name:
...
def download(file_url,download_location):
	...
	# NEW URLS post 2022 
	if "tableName" in file_url:
		if "dictionary" in file_url:
			file_name = file_url.split("tableName=")[-1] + "_Dict.zip"
		else:
			file_name = file_url.split("tableName=")[-1] + ".zip"
	else:
...
class IWD_file():
	def __init__(self,ipeds_file, year):
		...
		#URL CHANGE POST 2022
		if int(file_year) > 2022:
			self.data_file_dl = "https://nces.ed.gov/ipeds/data-generator?year={0}&tableName={1}".format(file_year, self.ipeds_file)
			self.data_dictionary_dl = "https://nces.ed.gov/ipeds/dictionary-generator?year={0}&tableName={1}".format(file_year, self.ipeds_file)
		else:
...
# For testing one survey

# main(year = new_year, file = "SFAV[YEAR]")
